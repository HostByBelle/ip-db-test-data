...
def process(json_file):
    ...
    with open(json_file, "r") as file:
        ...
        for entry in ip_data_list:
            ...
            if ip_network not in unique_ranges:
                if len(entry["country_code"]) == 3:
                    entry["country_code"] = convert_to_2_letter_code(
                        entry["country_code"]
                    )
                ...
                for kept_entry in result:
                    existing_range = ipaddress.ip_network(
                        kept_entry["ip_range"], strict=False
                    )
        ...
        print(
            f"There were {duplicatedCIDRs:,} duplicated, {overlappedCIDRs:,} overlapping, and {ignoredPrivateCIDRs:,} private CIDRs that were discarded."
        )
