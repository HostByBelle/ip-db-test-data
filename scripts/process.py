...
def process(json_file):
    ...
    with open(json_file, 'r') as file:
        ...
        # Sort supernets followed by their subnets
        ip_data_list.sort(key=lambda entry: (ipaddress.ip_network(entry["ip_range"], strict=False).num_addresses, -ipaddress.ip_network(entry["ip_range"], strict=False).prefixlen), reverse=True)
